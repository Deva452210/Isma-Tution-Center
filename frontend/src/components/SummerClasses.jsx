...
import { PhoneCall, CheckCircle2, Clock, BookOpen, Star, Sparkles, ChevronLeft, ChevronRight, MoveRight } from 'lucide-react';
    ...
    return (
        <section className="py-20 md:py-28 bg-gray-100  relative overflow-hidden">
            ...
            <div className="container mx-auto px-4 max-w-7xl relative z-10">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 max-w-7xl mx-auto gap-6 sm:gap-10">
                    ...
                    {/* Desktop Navigation & Mobile Hint */}
                    <div className="flex flex-col items-start md:items-end gap-3 shrink-0">
                        <div className="flex items-center gap-2 text-orange-600 font-bold text-[13px] uppercase tracking-wider md:hidden mb-2 ml-1">
                            Swipe to explore <MoveRight className="w-4 h-4" />
                        </div>
                        <div className="hidden md:flex items-center gap-3">
                            <button onClick={() => scroll('left')} className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-gray-700 shadow-md border border-gray-100 hover:text-orange-500 hover:border-orange-200 hover:shadow-lg transition-all" aria-label="Scroll Left">
                                <ChevronLeft className="w-6 h-6 -ml-0.5" />
                            </button>
                            <button onClick={() => scroll('right')} className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-gray-700 shadow-md border border-gray-100 hover:text-orange-500 hover:border-orange-200 hover:shadow-lg transition-all" aria-label="Scroll Right">
                ...
                >
                  ...
                  `}} />
                    {programs.map((program) => (
                        <div key={program.id} className={`flex-none w-[82vw] sm:w-[320px] lg:w-[280px] xl:w-[300px] snap-start rounded-3xl border p-8 flex flex-col transition-all duration-300 md:hover:shadow-xl md:hover:-translate-y-2 ${program.theme} relative overflow-hidden group`}>
