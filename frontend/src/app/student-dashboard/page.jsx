...
import { User, BookOpen, MapPin, Phone, Calendar, Hash, CheckCircle, Code, Award, Target, Flame, ChevronRight, Upload, X } from 'lucide-react';
...
export default function StudentDashboardPage() {
  ...
  const student = useMemo(() => {
    ...
    return studentsData.find(s => String(s.rollNumber) === String(rollNumber));
  }, [studentsData, rollNumber]);

  const isEligibleForMathLab = student && ['10', '11', '12'].includes(String(student.grade));

  // Edit Profile States
  const [isEditing, setIsEditing] = useState(false);
  const [editForm, setEditForm] = useState({});
  const [editPhotoFile, setEditPhotoFile] = useState({ base64: '', name: '', type: '', preview: null });
  ...
  const { data: questionsData, mutate: mutateQuestions } = useSWR(
    student && isEligibleForMathLab ? `${MATH_LAB_SCRIPT_URL}?action=getQuestions&grade=${student.grade}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );

  const { data: submissionsData, mutate: mutateSubmissions } = useSWR(
    rollNumber && isEligibleForMathLab ? `${MATH_LAB_SCRIPT_URL}?action=getSubmissions&rollNumber=${rollNumber}` : null,
    fetcher,
    { revalidateOnFocus: false }
  );
  ...
  // Compute stats
  const getQ = (subId) => questions.find(q => String(q._id) === String(subId) || q.title === String(subId));
  const solvedCount = submissions.filter(s => s.status === 'solved' && getQ(s.questionId)).length;
  const easySolved = submissions.filter(s => s.status === 'solved' && getQ(s.questionId)?.marks == 2).length;
  const medSolved = submissions.filter(s => s.status === 'solved' && getQ(s.questionId)?.marks == 5).length;
  const hardSolved = submissions.filter(s => s.status === 'solved' && getQ(s.questionId)?.marks == 10).length;
  ...
  const displayChapter = chapters.includes(mathLabChapter) ? mathLabChapter : chapters[0] || 'Chapter 1';
  ...
  return (
    <div className={`min-h-[calc(100vh-80px)] bg-[#0a0a0a] transition-colors duration-500`}>

      {/* Dashboard Top Nav */}
      <div className={`bg-[#1a1a1a] border-[#333] text-gray-300 border-b px-4 md:px-8 py-4 sticky top-0 z-40`}>
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-4">
            {student.photoUrl ? (
              <img src={getImageUrl(student.photoUrl)} alt="Profile" className="w-12 h-12 rounded-full object-cover border-2 border-[#333]" onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + student.name + '&background=random' }} />
            ) : (
              <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-xl ${activeTab === 'mathlab' ? 'bg-orange-500 text-white' : 'bg-brand text-white'}`}>
                {student.name.charAt(0)}
              </div>
            )}
            <div>
              <h2 className={`text-xl font-bold text-white`}>{student.name}</h2>
              <p className={`text-sm text-gray-400`}>Class {student.grade} • {student.rollNumber}</p>
            </div>
          </div>
          ...
          <div className="max-w-3xl mx-auto bg-[#1a1a1a] rounded-2xl shadow-lg border border-[#333] p-6 md:p-8">
            ...
            {isEditing ? (
              <form onSubmit={handleProfileUpdate} className="space-y-6">
                {updateStatus === 'success' && <div className="p-3 bg-green-50 text-green-700 rounded-lg">Profile updated successfully!</div>}
                {updateStatus === 'error' && <div className="p-3 bg-red-50 text-red-700 rounded-lg">Failed to update. Check console.</div>}

                {/* Editable Avatar */}
                <div className="flex flex-col items-center justify-center mb-8">
                  <div className="relative group cursor-pointer">
                    {editPhotoFile.preview || student.photoUrl ? (
                      <img src={editPhotoFile.preview || getImageUrl(student.photoUrl)} alt="Profile" className="w-32 h-32 rounded-full object-cover border-4 border-[#333]" onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + student.name + '&background=random' }} />
                    ) : (
                    ...
                    <input type="file" accept="image/*" onChange={handleEditPhotoChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" name="name" value={editForm.name || ''} onChange={handleEditChange} className="w-full border border-gray-200 rounded-xl p-3" required />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Phone</label>
                    <input type="tel" name="phone" value={editForm.phone || ''} onChange={handleEditChange} className="w-full border border-gray-200 rounded-xl p-3" required />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Address</label>
                    <textarea name="address" value={editForm.address || ''} onChange={handleEditChange} className="w-full border border-gray-200 rounded-xl p-3" rows="2" required></textarea>
                  </div>
                  <div>
                ...
                <div className="pt-4 flex justify-end">
                  <button type="submit" disabled={isUpdating} className="px-6 py-3 bg-brand text-white font-bold rounded-xl hover:bg-green-800 disabled:opacity-50">
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col items-center justify-center mb-8 border-b border-[#333] pb-8">
                  {student.photoUrl ? (
                    <img src={getImageUrl(student.photoUrl)} alt="Profile" className="w-32 h-32 rounded-full object-cover border-4 border-[#333] shadow-lg mb-4" onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + student.name + '&background=random' }} />
                  ) : (
                  ...
                  <p className="text-gray-400">Class {student.grade} • {student.rollNumber}</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <DetailItem icon={<BookOpen />} label="Class / Grade" value={student.grade} />
                  <DetailItem icon={<Calendar />} label="Date of Birth" value={student.dob ? student.dob.split('T')[0] : 'N/A'} />
                  <DetailItem icon={<MapPin />} label="Address" value={student.address} className="md:col-span-2" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="text-gray-300">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
                    <svg className="w-full h-full transform -rotate-90">
                      <circle cx="56" cy="56" r="52" stroke="#333" strokeWidth="8" fill="none" />
                      <circle cx="56" cy="56" r="52" stroke="#f97316" strokeWidth="8" fill="none"
                        strokeDasharray="326" strokeDashoffset={326 - (326 * solvePercentage) / 100}
                        strokeLinecap="round" className="transition-all duration-1000 ease-out" />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-3xl font-black text-white">{solvedCount}</span>
                      <span className="text-xs text-gray-500 font-medium tracking-widest uppercase">Solved</span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 space-y-3 w-full">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-white font-bold">{easySolved}<span className="text-gray-500">/{easyTotal}</span></span>
                    </div>
                    <div className="w-full bg-[#333] h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#00b8a3] h-full" style={{ width: `${easyTotal ? (easySolved / easyTotal) * 100 : 0}%` }}></div>
                    </div>

                    <div className="flex justify-between text-sm mt-3">
                      <span className="text-[#ffc01e]">Medium</span>
                      <span className="text-white font-bold">{medSolved}<span className="text-gray-500">/{medTotal}</span></span>
                    </div>
                    <div className="w-full bg-[#333] h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#ffc01e] h-full" style={{ width: `${medTotal ? (medSolved / medTotal) * 100 : 0}%` }}></div>
                    </div>

                    <div className="flex justify-between text-sm mt-3">
                      <span className="text-[#ff375f]">Hard</span>
                      <span className="text-white font-bold">{hardSolved}<span className="text-gray-500">/{hardTotal}</span></span>
                    </div>
                    <div className="w-full bg-[#333] h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#ff375f] h-full" style={{ width: `${hardTotal ? (hardSolved / hardTotal) * 100 : 0}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#1a1a1a] rounded-2xl border border-[#333] p-6 flex flex-col shadow-lg">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-gray-400 font-semibold flex items-center gap-2">
                    <Award className="w-4 h-4 text-purple-500" /> Badges & Streaks
                  </h3>
                  <span className="bg-[#333] text-gray-300 text-xs px-2.5 py-1 rounded-full font-bold">2 Badges</span>
                </div>
                ...
                <div className="flex gap-4">
                  {easySolved >= 5 ? (
                    <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#00b8a3]/20 to-[#00b8a3]/5 border border-[#00b8a3]/30 flex flex-col items-center justify-center shadow-[0_0_15px_rgba(0,184,163,0.15)]">
                      ...
                      <span className="text-[10px] text-[#00b8a3] font-bold">Novice</span>
                    </div>
                  ) : (
                    <div className="w-16 h-16 rounded-xl bg-[#2a2a2a] border border-[#333] flex flex-col items-center justify-center opacity-50 grayscale">
                      ...
                      <span className="text-[10px] text-gray-500 font-bold">Novice</span>
                    </div>
                  )}
                  ...
                  {medSolved >= 3 ? (
                    <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#ffc01e]/20 to-[#ffc01e]/5 border border-[#ffc01e]/30 flex flex-col items-center justify-center shadow-[0_0_15px_rgba(255,192,30,0.15)]">
                      ...
                      <span className="text-[10px] text-[#ffc01e] font-bold">Adept</span>
                    </div>
                  ) : (
                    <div className="w-16 h-16 rounded-xl bg-[#2a2a2a] border border-[#333] flex flex-col items-center justify-center opacity-50 grayscale">
                      <Flame className="w-6 h-6 text-gray-500 mb-1" />
                      <span className="text-[10px] text-gray-500 font-bold">Adept</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-[#1a1a1a] rounded-2xl border border-[#333] p-6 lg:col-span-1 shadow-lg hidden lg:block bg-[url('https://cdn.pixabay.com/photo/2016/11/29/05/45/astronomy-1867616_1280.jpg')] bg-cover bg-center relative group">
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <h3 className="text-white font-bold text-xl mb-2">Math is the language of the universe.</h3>
                  <p className="text-gray-400 text-sm">Keep solving, keep growing.</p>
                  <div className="mt-auto">
                    <button className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm font-semibold backdrop-blur-sm transition-all border border-white/10">Daily Challenge →</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#1a1a1a] rounded-2xl border border-[#333] p-6 mb-8 shadow-lg overflow-hidden">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center text-sm text-gray-400 mb-6 gap-4">
                <div>
                  <span className="text-white font-bold text-lg">{totalSubmissions}</span> submissions in the past one year
                </div>
                <div className="flex flex-wrap gap-4 md:gap-6 text-xs md:text-sm">
                  <div>Total active days: <span className="text-white font-bold">{activeDays}</span></div>
                  <div>Max streak: <span className="text-white font-bold">{maxStreak}</span></div>
                  <div>Current streak: <span className="text-white font-bold">{currentStreak}</span></div>
                </div>
              </div>
                ...
                <div className="min-w-max">
                  <div className="flex gap-1 mb-2">
                    {weeks.map((week, wIndex) => (
                      <div key={wIndex} className="flex flex-col gap-1">
                        {week.map((day, dIndex) => {
                          if (!day) return <div key={dIndex} className="w-3 h-3 bg-transparent"></div>;
                  ...
                  <div className="relative h-6 text-xs text-gray-500 font-medium">
                    {monthLabels.map((ml, i) => (
                      <div key={i} className="absolute top-1" style={{ left: `${ml.weekIndex * (12 + 4)}px` }}>
                        {ml.name}
                      </div>
                    ))}
                  </div>
            ...
            <div className="bg-[#1a1a1a] rounded-2xl border border-[#333] shadow-lg overflow-hidden">
              ...
              <div className="flex border-b border-[#333] bg-[#222] overflow-x-auto custom-scrollbar-dark">
                {chapters.length === 0 ? (
                  <div className="p-4 text-sm text-gray-500">No chapters found. Wait for admin to upload questions.</div>
                ) : (
                  chapters.map(ch => (
                    <button
                      key={ch}
              ...
              <div className="p-0">
                <table className="w-full text-left text-sm text-gray-400">
                  ...
                  <tbody className="divide-y divide-[#333]/50">
                    {questions.filter(q => q.chapterName === displayChapter).length === 0 ? (
                      <tr><td colSpan="4" className="px-6 py-10 text-center">No problems available in this chapter.</td></tr>
                    ) : (
                      questions.filter(q => q.chapterName === displayChapter).map((q, idx) => {
                        const marks = parseInt(q.marks);
                        let diffColor = 'text-[#00b8a3]';
                        let diffText = 'Easy (2M)';
                        if (marks === 5) { diffColor = 'text-[#ffc01e]'; diffText = 'Med. (5M)'; }
                        if (marks >= 10) { diffColor = 'text-[#ff375f]'; diffText = 'Hard (10M)'; }

                        // Check status
                        const sub = submissions.find(s => s.questionId === q._id);
                        let statusIcon = <div className="w-4 h-4 rounded-full border border-[#555]"></div>;
                        if (sub && sub.status === 'solved') {
                          statusIcon = <CheckCircle className="w-5 h-5 text-orange-500" />;
                        } else if (sub && sub.status === 'pending') {
                          statusIcon = <div className="w-4 h-4 rounded-full border-2 border-[#ffc01e] border-t-transparent animate-spin"></div>;
                        }

                        return (
                          <tr key={q._id || idx} className="hover:bg-[#222] transition-colors group">
                            <td className="px-6 py-4">{statusIcon}</td>
                            <td className="px-6 py-4">
                              <Link href={`/student-dashboard/problem/${q._id || encodeURIComponent(q.title)}`} className="font-semibold text-gray-200 group-hover:text-orange-500 transition-colors cursor-pointer block">{q.title}</Link>
                              {q.description && <p className="text-xs text-gray-600 truncate max-w-sm mt-1">{q.description}</p>}
                            </td>
                            <td className={`px-6 py-4 font-bold ${diffColor}`}>{diffText}</td>
                            <td className="px-6 py-4 text-right">
                              {sub ? (
                                <span className={`text-xs font-bold px-2.5 py-1 rounded-md ${sub.status === 'solved' ? 'bg-[#00b8a3]/10 text-[#00b8a3]' : 'bg-[#ffc01e]/10 text-[#ffc01e]'}`}>
                                  {sub.status === 'solved' ? 'Solved' : 'Pending Verification'}
                                </span>
                              ) : (
                                <Link
                                ...
                                >
                                  Solve <ChevronRight className="w-3 h-3 ml-1" />
                                </Link>
                              )}
                            </td>
                          </tr>
...
function DetailItem({ icon, label, value, className = '' }) {
  return (
    <div className={`flex items-start p-4 rounded-xl border border-[#333] bg-[#282828] ${className}`}>
      <div className="p-2 bg-[#3d3d3d] rounded-lg text-gray-300 shadow-sm mr-4 shrink-0">
        {icon}
      </div>
      <div>
        <p className="text-sm text-gray-400 font-semibold mb-1 uppercase tracking-wider text-xs">{label}</p>
